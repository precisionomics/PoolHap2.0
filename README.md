...
3. Similarly, FS\*.properties sets the simulation parameters for the simulation wrapper program FullSimulator (FS2.jar for 'perfect' or FS3.jar for 'non-perfect'). Update the absolute paths of the directories and programs. Place this in `input/`. 
...
7. The result files (allele composition and within/across-pool frequencies) will show up in `output/`. The gold-standard files, which are generated by FS2/3.jar, will be in `gold_standard/`. 
