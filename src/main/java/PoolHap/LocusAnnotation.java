...
public class LocusAnnotation {
	 ...
	 */
	boolean is_region; // whether this is a primitive locus (e.g., a SNP or indel) or a region that
					   // contains multiple primitive loci
	int chr_index; // index of the chromosome, starting from ZERO
	int start_loc;
	int end_loc; // for SNPs and indels, end_loc = start_loc; start_loc != end_loc only if it is
				 // a region, instead of a primitive locus
