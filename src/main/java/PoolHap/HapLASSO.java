...
public class HapLASSO {
                ...
                for (int loc2 = loc1 + 1; loc2 < this.num_loci; loc2++) {
                    ...
                    if (!Double.isNaN(this.ld_matrix[loc1][loc2])) {
                        ...
                        for (int h = 0; h < this.potential_haps.num_global_hap; h++) {
                            ...
                            // TODODONE: Lauren: please double-check whether Quan's line below is
                            // correct!!!!
    ...
    public void conduct_regression() throws FileNotFoundException {
        ...
        // TODODONE: Quan to Lauren: I think setFitIntercept(false) will ensure that the regression
    ...
    public void estimate_frequencies_lasso(
        ...
        if (vef_file != null && vef_files == null) { // there is a vef_file, indicating single pool
            ...
            // TODO: in the future, we may have a version use both stat- and read-based ld_matrix
            // and a parameter to structure their relative weights.
        } else {
            System.out.println("ERROR: Combination of parameters wrong!");
        }

        // Set up weights after calculating Ys.
        // TODO: note that setup_weights() should be implemented differently based on the type of
        // calculating ld_matrix in the future.
