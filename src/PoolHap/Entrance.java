...
public class Entrance {
	...
	/**
	 * Reconstructed by @author quanlong 2019-07. 
	 * (1) split into three modules 
	 * (2) The users need to put input files into directories named as sam, vef, and vcf, instead of 
	 * 		specified prefix plus "_p" and number. Variables [name2index] and [names_array] will take 
	 * 		over the numbering and indexing. The possible suffixes are specified in the variable
	 *      [suffixes]   
	 */
	
	public static String[] suffixes= {"sam", "vcf", "vef", "gcf"};
     ...
     * 
     * Get all file paths from a folder. All the files should have the same suffix and the number
     * of files must be the same as the number of pools. All files should have the same names
     * 
     * If @param initialize_names ==true, the three static fields will be initialized.
     * Otherwise, the established values in the three fields will be compared with the outcome
     * for a double-check.  
     */
	
	public static String[] get_filepaths(String name_file, String folder, String suffix, 
    		...
    		if(write_name_file) { // write the name_file and initiate structures. 
    			...
    			for(int index=0;index<Entrance.num_pools;index++) {
    				...
    				if(!separated[separated.length-1].equals(suffix)) {
    					...
    					if(separated.length>=3) { //except for [0] and the suffix, there are elements
    			...
    			for(int index=0;index<Entrance.num_pools;index++) {
    				String[] separated=observed_files[index].split("/")
    						[observed_files[index].split("/").length-1].split("\\.");
    				if(!separated[separated.length-1].equals(suffix)) {
    					System.out.println("ERROR: The suffix of "+observed_files[index]+" is not "+suffix); 
        				System.exit(0);
    				}else {
    					String name=separated[0]; // get the string before the suffix, i.e., name
    					if(separated.length>=3) { //except for [0] and the suffix, there are elements
    					...
    					if(!Entrance.name2index.containsKey(name)) {
    						...
    						System.out.println("ERROR: The name of "+observed_files[index]+" is not in "+
    ...
    public static void main(String[] args) throws Exception {
        ...
        // removed by Quan Long 2019-07-03. This will be identified by counting how many files in a folder
        ...
        String gs_var_pos = gp.inter_dir + gp.project_name + "_vars.intra_freq.txt"; //TODO: change name
        ...
        if (gp.function.equals("format")){
        	// SAM files in input_dir/sam initiates pool-IDs and orders, and write to the "name_file".
        	// The function below generates vef files and inpool-var-freq file.
        	String[] sam_files=Entrance.get_filepaths(name_file, gp.input_dir+"/sam/", "sam", true);
        	BAMFormatterGATK.generate_vef_varfreq(gp.input_dir, gp.inter_dir, gp.project_name, 
        			sam_files, Entrance.name2index);
        }
...
//                new FileWriter(new File(gp.inter_dir + prefix + "_p.in.list")));
        	...
        	String dc_out_file = gp.inter_dir + gp.project_name + "_dc_plan.txt"; // dc output filepath string
            ...
            final_global_haps = region_linker.hapOut(Entrance.names_array); // HapConfig from GC-linked regions
...
//            String prefix=null; 
           for (int pool_index = 0; pool_index < num_pools; pool_index++) {
               ...
               inpool_lasso.estimate_frequencies_lasso(
                   gp.inter_dir + "/vef/" + Entrance.names_array[pool_index] + ".vef",
                   null,
                   gp.lasso_weights);
               ...
               final_inpool_haps[pool_index] = inpool_lasso.hapOut(); // add final intra pool HapConfig to array
               System.out.println(final_inpool_haps[pool_index].num_global_hap
                   + " haplotypes were generated for pool "
                   + pool_index+":"+Entrance.names_array[pool_index]
                   + ".");
