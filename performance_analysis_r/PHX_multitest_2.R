...
# GC, rjMCMC and PHX prop_error ~ orig_haps + ver_var_pos + num_haps
...
for (m in 1:length(moduleTypes)) {
	...
	tmp = setNames(data.frame(t(stats_data[c(1,3),-c(1)])), t(stats_data[c(1,3),-c(1)])[1,])[-1,]
	tmp$num_haps = tmp_haps
	tmp$prop_error = tmp_err
	tmp_df1 = transform(tmp, orig_haps = as.numeric(as.character(orig_haps)), ver_var_pos = as.numeric(as.character(ver_var_pos)))
	tmp_df = tmp_df1[(tmp_df1$num_haps != 0), ]	
	tmp_lm = lm(prop_error ~ orig_haps + ver_var_pos, data = tmp_df)
	print(mod)
